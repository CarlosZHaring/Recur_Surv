...
ggsurvplot(KP_race, data = clinData)
ggsurvplot(KP_meta, data = clinData)
```
...
ggsurvplot(KP_tumortype, data = clinData)
...
```{r,fig.height=20,fig.width=20}
cox_stage <- coxph(Surv_obj ~ Staging.Tumor.Size....T. + Staging.Nodes...Nx.replaced.by..1..N. + Staging.Metastasis...Mx..replaced.by..1..M., data = clinData)
...
ggsurvplot(KP_T, data = clinData)
ggsurvplot(KP_N, data = clinData)
ggsurvplot(KP_M, data = clinData)
```
...
ggsurvplot(KP_nott, data = clinData)
```

```{r,fig.height=20,fig.width=20}
table(clinData$Histologic.type)

cox_his <- coxph(Surv_obj ~ Histologic.type, data = clinData)
summary(cox_his)

KP_his <- survfit(Surv_obj ~ Histologic.type, data = clinData)
ggsurvplot(KP_his, data = clinData)
```
...
cox_bi <- coxph(Surv_obj ~ factor(Bilateral.Information), data = clinData)
summary(cox_bi)
```
...
clinData <- clinData %>%
  mutate(
    ...
    Days.to.Surgery = ifelse(
      ...
      NA,  # 或者你可以写 0 或 "abs()" 或另设一列记录为负
      Days.to.Surgery..from.the.date.of.diagnosis.
    )
  )

cox_surgery <- coxph(Surv_obj ~ Surgery + Days.to.Surgery + Definitive.Surgery.Type, data = clinData)
summary(cox_surgery)
```
