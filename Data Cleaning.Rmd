...
data <- read.csv("clinicalData_clean.csv")
...
data <- data %>%
  mutate(
    Days.to.death..from.the.date.of.diagnosis. = as.numeric(na_if(Days.to.death..from.the.date.of.diagnosis., "NP"))
  )
...
# Age (selected)
data$Date.of.Birth..Days. <- round(-data$Date.of.Birth..Days. / 365.25, 1) 

# Race (selected)
data <- data %>% mutate(
  Race.and.Ethnicity = as.factor(case_when(Race.and.Ethnicity == 1 ~ "White", 
                                                  Race.and.Ethnicity == 2 ~ "Black",
                                                  Race.and.Ethnicity %in% c(3, 4, 5, 6, 7) ~ "Other"))
  )

# Metastatic at Presentation (Selected)
# Important but Sparse: Subgroup
data <- data %>% mutate(
  Metastatic.at.Presentation..Outside.of.Lymph.Nodes. = 
    case_when(Metastatic.at.Presentation..Outside.of.Lymph.Nodes. == 0 ~ "No",
              Metastatic.at.Presentation..Outside.of.Lymph.Nodes. == 1 ~ "Yes")
  )
...
# Oncotype DX (selected)
# 72% missing:Stratify the Oncotype or Imputation
...
data <- data %>%
  mutate(
    ...
    Definitive.Surgery.Type = as.factor(
      case_when(Definitive.Surgery.Type == 0 ~ "BCS",
              Definitive.Surgery.Type == 1 ~ "mastectomy")
  ),
...
cleaned_data <- data %>% select(
  ...
  rename(
    ...
    Days_to_death = Days.to.death..from.the.date.of.diagnosis.,
